...
void imageCompositer(cv::Mat* main, cv::Mat* image1, cv::Mat* image2, cv::Mat* image3, double fps, int WIDTH, int HEIGHT, std::queue<cv::Mat>* imQueue, double rAlpha, double rBeta, double lAlpha, double lBeta)
{
    bool firstLoop=true;
    ...
    *main = cv::Mat(cv::Size(WIDTH*2, HEIGHT*2), CV_8UC3);
    ...
    while(!endThreads)
    {
        ...
        //changes brightness for the elp cameras
	    image1->convertTo((*main)(cv::Rect(0,0,WIDTH,HEIGHT)), -1, lAlpha, lBeta);
	    image2->convertTo((*main)(cv::Rect(WIDTH,0,WIDTH,HEIGHT)), -1, rAlpha, rBeta);
	    
	    //dewarper.dewarpFrame(*image1).convertTo((*main)(cv::Rect(0,0,WIDTH,HEIGHT)), -1, 1.5, 80);
	    //dewarper.dewarpFrame(*image2).convertTo((*main)(cv::Rect(WIDTH,0,WIDTH,HEIGHT)), -1, 1.5, 80);
...
void imageWriter(cv::VideoWriter* writer,  std::queue<cv::Mat>* imQueue)
{
    ...
    while(true)
    {
        ...
        imName = "images/img" + ss.str() + ".jpg";
        
        imwrite(imName, imQueue->front());
        imQueue->pop();
        //cv::waitKey(1);
        ++counter;
    }
...
int main(int argc, char* argv[])
{
    ...
    std::thread compositer(&imageCompositer, &fullImage, &frame1, &frame2, &frame3, FPS, WIDTH, HEIGHT, images, rightAlpha, rightBeta, leftAlpha, leftBeta);
    compositer.detach();
