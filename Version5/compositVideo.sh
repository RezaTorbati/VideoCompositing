...
TR_ALPHA=$(cat config.txt | grep TOP_RIGHT_ALPHA | sed 's/^.*=//')
TR_BETA=$(cat config.txt | grep TOP_RIGHT_BETA | sed 's/^.*=//')
TL_ALPHA=$(cat config.txt | grep TOP_LEFT_ALPHA | sed 's/^.*=//')
TL_BETA=$(cat config.txt | grep TOP_LEFT_BETA | sed 's/^.*=//')
...
$STARTING_DIR/build/videoCompositer $TL_FILE $TR_FILE $BL_FILE $FPS $TR_ALPHA $TR_BETA $TL_ALPHA $TL_BETA
